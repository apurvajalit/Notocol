...
namespace Repository.Search
{
    ...
    public class ElasticSearchTest
    {
        ...
        public void AddUser(User user)
        {
            ...
            esuser.id = user.ID;
            esuser.user_suggest.Input = new string[] { user.Username };
        ...
        public List<PageDetails> GetLongSource(SourceListFilter filter, long userID, int sourceType, int offset, int size)
        {
            List<PageDetails> results = new List<PageDetails>();
            bool applyTagFilter = (filter.tags != null) ? ((filter.tags.Length > 0) ? true : false) : false;
            bool applyUserFilter = (filter.user != null && filter.user.Length > 0) ? true : false;
            List<ESSource> source = new List<ESSource>();
            QueryContainer tagsQuery = new TermsQuery
            ...
            if (searchResponse.Hits.Any())
            {
                ...
                foreach (var hit in searchResponse.Hits)
                {
                    PageDetails s = new PageDetails
                    {
                        faviconURL = hit.Source.faviconURL,
                        Id = hit.Source.Id,
                        tags = hit.Source.tags != null ? hit.Source.tags.ToList() : null,
                        thumbnailImageUrl = hit.Source.tnImage,
                        thumbnailText = hit.Source.tnText,
                        title = hit.Source.title,
                        url = hit.Source.link,
                        users = hit.Source.publicUserNames != null ? hit.Source.publicUserNames.ToList() : null
                    };
                    
                    if (hit.InnerHits.Any())
                    {
                        s.notes = new List<NoteDisplay>();
                        foreach (var innerHit in hit.InnerHits)
                        {
                            if (!innerHit.Value.Hits.Hits.Any()) continue;
                            foreach (var childHighlightHolder in innerHit.Value.Hits.Hits)
                            {
                                ...
                                foreach (var highlight in childHighlightHolder.Highlights.SelectMany(highlight => highlight.Value.Highlights))
                                {
                                    s.notes.Add(new NoteDisplay {quote = highlight });
